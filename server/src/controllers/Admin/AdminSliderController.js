...
exports.saveOfferSlider = async (req, res) => {
  try {
    const { description, validTill, img, applyToAll, products } = req.body;

    // Determine final values based on applyToAll flag
    const offerData = {
      description,
      validTill,
      image: img,
    ...
    if (applyToAll) {
      offerData.applyToAll = true;
      offerData.products = []; // Ensure product list is cleared
    } else if (Array.isArray(products) && products.length > 0) {
      offerData.applyToAll = false;
      offerData.products = products;
    } else {
      // Fallback: if no products provided, default to applyToAll
      offerData.applyToAll = true;
      offerData.products = [];
    }
